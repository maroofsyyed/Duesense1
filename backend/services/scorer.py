"""Investment Scoring System - compiles all agent scores into final recommendation."""
import asyncio
...
async def calculate_investment_score(company_id: str, extracted: dict, enrichment: dict) -> dict:
    """Run all 5 agents in parallel and compile final score."""
    ...
    founder_score = min(30, max(0, float(founder_result.get("total_founder_score", 15))))
    ...
    traction_score = min(20, max(0, float(traction_result.get("total_traction_score", 10))))
    model_score_val = min(10, max(0, float(model_result.get("total_model_score", 5))))

    total = founder_score + market_score + moat_score + traction_score + model_score_val
    ...
    thesis = await _generate_thesis(extracted, total, tier, founder_result, market_result, moat_result, traction_result, model_result)
...
async def _generate_thesis(extracted, total, tier, founder, market, moat, traction, model) -> dict:
...
- Founders: {founder.get('total_founder_score', 'N/A')}/30 - {founder.get('reasoning', 'N/A')[:200]}
- Market: {market.get('total_market_score', 'N/A')}/20 - {market.get('reasoning', 'N/A')[:200]}
- Moat: {moat.get('total_moat_score', 'N/A')}/20 - {moat.get('reasoning', 'N/A')[:200]}
- Traction: {traction.get('total_traction_score', 'N/A')}/20 - {traction.get('reasoning', 'N/A')[:200]}
- Business Model: {model.get('total_model_score', 'N/A')}/10 - {model.get('reasoning', 'N/A')[:200]}

COMPANY INFO:
{_safe_json(extracted.get('company', {}))}
...
{{
  ...
  "expected_return": "Nx in Y years"
    ...
    return await llm.generate_json(prompt, "You are a senior VC partner writing investment recommendations.")
